...
login_pattern = re.compile(
    r"INFO\s+: LoginAction: ServiceClientLoginAction: client token:"
)
service_registration_pattern = re.compile(
    r"INFO\s+: LoginAction: RMIProcessClient: created new RMIProcessClient 'ClientCLIService'"
)
...
def resolve_rmi_port():
    ...
    if str(raw).isdigit():
        return int(raw)
    print(
        f"Warning: SERVICECLIENT_RMI_PORT '{raw}' is not numeric; falling back to {DEFAULT_RMI_PORT}."
    )
